...
pub struct ProcessWormholeMessage<'info> {
    ...
    //     seeds::program = state.wormhole_program,
    //     bump,
    //     constraint = wormhole_message.consistency_level >= 1 @ NftVerificationError::InvalidConsistencyLevel,
    // )]
    // pub wormhole_message: Account<'info, PostMessageData>,
    ...
    #[account(
        ...
        seeds = [
            b"proof",
            // wormhole_message.payload[20..40].as_ref(),  // NFT contract address
            // wormhole_message.payload[40..72].as_ref(),  // Token ID
        ],
        bump
    )]
...
pub struct ClaimTokens<'info> {
    ...
    #[account(
        ...
        seeds = [
            b"proof",
            proof_record.nft_contract.as_ref(),
