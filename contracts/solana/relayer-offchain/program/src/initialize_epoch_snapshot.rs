...
use relayer_ncn_core::{
    config::Config, epoch_snapshot::EpochSnapshot, error::RelayerNcnError,
...
pub fn process_initialize_epoch_snapshot(
    ...
    let vault_count = {
        ...
        if !weight_table_account.finalized() {
            ...
            return Err(RelayerNcnError::WeightTableNotFinalized.into());
        }
    ...
    if operator_count == 0 {
        ...
        return Err(RelayerNcnError::NoOperators.into());
    }
