...
impl EpochAccountStatus {
    ...
    pub const fn get_account_status(u: u8) -> Result<AccountStatus, RelayerNcnError> {
        match u {
            0 => Ok(AccountStatus::DNE),
            1 => Ok(AccountStatus::Created),
            2 => Ok(AccountStatus::CreatedWithReceiver),
            3 => Ok(AccountStatus::Closed),
            _ => Err(RelayerNcnError::InvalidAccountStatus),
        }
    }

    pub const fn epoch_state(&self) -> Result<AccountStatus, RelayerNcnError> {
        Self::get_account_status(self.epoch_state)
    }

    pub const fn weight_table(&self) -> Result<AccountStatus, RelayerNcnError> {
        Self::get_account_status(self.weight_table)
    }

    pub const fn epoch_snapshot(&self) -> Result<AccountStatus, RelayerNcnError> {
        Self::get_account_status(self.epoch_snapshot)
    }

    pub const fn operator_snapshot(&self, index: usize) -> Result<AccountStatus, RelayerNcnError> {
        Self::get_account_status(self.operator_snapshot[index])
    }

    pub const fn ballot_box(&self) -> Result<AccountStatus, RelayerNcnError> {
        Self::get_account_status(self.ballot_box)
    }

    pub const fn base_reward_router(&self) -> Result<AccountStatus, RelayerNcnError> {
    ...
    //     index: usize,
    //     group: NcnFeeGroup,
    // ) -> Result<AccountStatus, RelayerNcnError> {
...
impl Progress {
    ...
    pub fn increment_one(&mut self) -> Result<(), RelayerNcnError> {
        self.increment(1)
    }

    pub fn increment(&mut self, amount: u64) -> Result<(), RelayerNcnError> {
...
impl EpochState {
    ...
    pub fn load_to_close(
        ...
        if account_to_close.ncn().ne(ncn) {
            ...
            return Err(RelayerNcnError::CannotCloseAccount.into());
        }
        ...
        if !account_to_close.account_status.are_all_closed() {
            ...
            return Err(RelayerNcnError::CannotCloseEpochStateAccount.into());
        }
    ...
    pub fn load_and_check_is_closing(
        ...
        if account_struct.is_closing() {
            ...
            return Err(RelayerNcnError::EpochIsClosingDown.into());
        }
    ...
    pub fn get_slot_consensus_reached(&self) -> Result<u64, RelayerNcnError> {
    ...
    pub fn update_set_merkle_root(&mut self) -> Result<(), RelayerNcnError> {
    ...
    pub fn update_distribute_base_rewards(&mut self, rewards: u64) -> Result<(), RelayerNcnError> {
